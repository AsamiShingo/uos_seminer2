...
個人の目標（ハイパーマンダラート）を1on1・分析とひと続きにし、「目標の立てっぱなし・1on1のやりっぱなし」をなくす組織成長プラットフォームという設定の架空アプリです。第2回セミナーでは、このアプリのLPを実際の開発フローに沿って作成・修正します。

- 初版の仕様は [design/アプリ設計.md](design/アプリ設計.md)、見た目の指定は [design/DESIGN.md](design/DESIGN.md) にまとまっています。
- 顧客デモ後の指摘に基づく仕様変更は [design/アプリ設計2_顧客指摘.md](design/アプリ設計2_顧客指摘.md) にまとめられています。[design/Mandalart LP.dc.html](design/Mandalart%20LP.dc.html) はこの指摘を反映済みで、以下の点が初版から変わっています。
  - 機能モジュールを6つ（マンダラート／1on1／タスク管理／経歴・スキル管理／お知らせ／成長分析）から、タスク管理・経歴・スキル管理を廃止した4つ（ハイパーマンダラート／1on1／お知らせ・通知／成長分析）に縮小
  - マンダラートを9×9＝81セルから10×10＝100セルへ拡張し、「ハイパーマンダラート」に改称
  - 外部連携をOutlookカレンダー連携に加えてSlack連携を追加
